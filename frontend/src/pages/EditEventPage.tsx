...
function EditEventPage() {
  ...
  const handleUpdateEventDescription = async () => {
    try {
      const res = await api.put(`/api/events/${eventId}/description`, {
        eventDescription: (document.getElementById("event-description") as HTMLTextAreaElement | null)?.value ?? "",
      });
      toast.success(res.data.message);
      navigate(`/organizers/${user?._id}/organize`);
    } catch (error) {
  ...
  return (
    <div className="p-5 w-full lg:max-w-[60%] mx-auto lg:mt-10 mt-5">
          ...
          <h3 className="font-normal lg:text-lg text-sm mt-2 mb-10">{formattedDate}</h3>
          ...
          <div className="font-normal text-lg mt-3 mb-10">
            <textarea className="w-full h-fit border-2 border-green-300 bg-gray-50 p-4 rounded text-sm lg:text-lg" defaultValue={event?.eventDescription} id="event-description" rows={8}>
            </textarea>
          </div>
          <div className="flex mb-20 gap-8">
            <button className="bg-blue-400 text-white p-2 rounded cursor-pointer hover:bg-blue-500" onClick={handleUpdateEventDescription}>Save Changes</button>
            <button className="bg-red-400 text-white p-2 rounded cursor-pointer hover:bg-red-500" onClick={handleDeleteEvent}>Delete Event</button>
          </div>
        </div>
  )
}
